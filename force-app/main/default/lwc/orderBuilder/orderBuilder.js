...
function getQuantity(orderItem) {
    var getSObjectVal=getSObjectValue(orderItem, QTY_SMALL_FIELD)
    console.log('getSObjectValue'+getSObjectValue(orderItem, QTY_SMALL_FIELD));
    console.log('getSObjectVal'+getSObjectVal);
...
function calculateOrderSummary(orderItems) {
    ...
    console.table(orderItems.Product__r);
    console.table(JSON.stringify(orderItems));
    const summary = orderItems.reduce(
        (acc, orderItem) => {
            ...
            console.log('acc.quantity'+acc.quantity);
            acc.price += price;
...
export default class OrderBuilder extends LightningElement {
    ...
    handleDrop(event) {
        ...
        console.log('handleDropproduct'+product);
